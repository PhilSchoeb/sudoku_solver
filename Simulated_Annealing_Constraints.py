...
# le numéro 5 : Simulated annealing avec la propagation des contraintes
...
def solve_all(grids, name='', showif=0.0):
    ...
    def solve_single(grid):
        ...
        print("Using Simulated Annealing, went from %s to %s" % (initial_score, final_score))
    ...
    if N >= 1:
        ...
        print("Solved %d of %d %s puzzles, and %d were solved without Simulated Annealing, (avg %.8f secs (%d Hz), "
              "max %.8f secs), average score from : %s to %s." % (sum(results), N, name, sum(already), sum(times)/N,
